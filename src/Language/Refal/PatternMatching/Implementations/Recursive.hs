...
matchPattern' subs@Substitutions {eType = es} ((PVar (EType v)) : ps) o =
  ...
  where
    withEVar (elts, rest) = matchPattern' (pushEType v elts subs) ps rest
